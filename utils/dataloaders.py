...
def full_test_loader(data_dir):
    ...
    return test_dataset # return test_dataset(type: dict)

def full_demo_loader(data_dir):

    demo_data = [i for i in os.listdir(data_dir + 'A/') if not
                    i.startswith('.')]
    demo_data.sort()

    demo_label_paths = []
    for img in demo_data:
        demo_label_paths.append(data_dir + 'OUT/' + img)

    demo_data_path = []
    for img in demo_data:
        demo_data_path.append([data_dir, img])

    demo_dataset = {}
    for cp in range(len(demo_data)):
        demo_dataset[cp] = {'image': demo_data_path[cp],
                           'label': demo_label_paths[cp]}
    # print(test_dataset)
    

    return demo_dataset # return test_dataset(type: dict)
...
def cdd_loader(img_path, label_path, aug):
    ...
    if dir == "./datasets/demo/": # Create pseudo label for demo image
        psuedolabel = np.zeros((img1.size[0], img1.size[1]), dtype=np.int8)
        psuedolabel_pil = Image.fromarray(psuedolabel)
        psuedolabel_pil.save(label_path, "PNG")
...
class CDDloader(data.Dataset):
    ...
    def __getitem__(self, index): # index is given from torch.dataloader
