...
from models.utilforBN import SynchronizedBatchNorm2d
...
class HRNetV2_ECAM(nn.Module):
    def __init__(self): # def __init__(self, n_class, **kwargs):
        super(HRNetV2_ECAM, self).__init__()
        ...
        self.conv1 = nn.Conv2d(6, 64, kernel_size=3, stride=2, padding=1,
                               bias=False) # concat: 6 to 64 / subtract: 3 to 64
        self.bn1 = BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1,
                               bias=False) # original : kernel_size=3, stride=2, padding=1
        ...
        self.conv_reshape1 = conv1x1(96, 48)
        self.conv_reshape2 = conv1x1(192, 48)
        self.conv_reshape3 = conv1x1(384, 48)

        # ECAM part 
        self.ca = ChannelAttention(48 * 4, ratio=16)
        self.ca1 = ChannelAttention(48, ratio=16 // 4)
        self.conv_final = nn.Conv2d(48 * 4, 2, kernel_size=1)
    ...
    def forward(self, xA, xB, return_feature_maps=False):
        """
        original: x is only one
        concat: concat xA and xB
        subtract: subtract xB from xA
        """
        x = torch.cat([xA, xB], 1) # concat
        # x = torch.abs(torch.sub(xA, xB)) # subtract
        # print(x.shape)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x1 = self.conv_reshape1(x1)
        x2 = self.conv_reshape2(x2)
        x3 = self.conv_reshape3(x3)

        # ECAM
        out = torch.cat([x[0], x1, x2, x3], 1)
        intra = torch.sum(torch.stack((x[0], x1, x2, x3)), dim=0)
        ca1 = self.ca1(intra)
        ...
        out = F.interpolate(
            out, size=(xA.size(2), xA.size(3)), mode='bilinear', align_corners=False)
