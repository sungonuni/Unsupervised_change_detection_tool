...
def pca_change_classification(img1, img2, gt):
    ...
    # Do UMAP
    features = umap.UMAP(n_components=3, random_state=0).fit_transform(diff_conv_vectors)
...
def mk_3dscatter(dataset, center_of_total_uc, center_of_total_c, uc_dict_per_img, c_dict_per_img):
    ...
    ax.view_init(0,0)
    plt.savefig(dataset+'_figure_umap_YZ.png')
    ax.view_init(0,90)
    plt.savefig(dataset+'_figure_umap_XZ.png')
    ax.view_init(90,90)
    plt.savefig(dataset+'_figure_umap_XY.png')
    print('figure saved')
